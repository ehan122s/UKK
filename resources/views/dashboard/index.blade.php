    ...
    @if ($terbaru)
        <div class="card">
            ...
            <ul class="info-list">
                ...
                <li><span>Total Penghasilan</span><span>Rp {{ number_format($terbaru->total_penghasilan ?? 0, 0, ',', '.') }}</span></li>
                <li><span>Total Potongan</span><span>Rp {{ number_format($terbaru->total_potongan ?? 0, 0, ',', '.') }}</span></li>
                <li><span>Gaji Bersih</span><span><strong>Rp {{ number_format($terbaru->gaji_bersih ?? 0, 0, ',', '.') }}</strong></span></li>
            </ul>
            <a href="{{ route('gaji.slip', $terbaru) }}" class="link">Lihat slip ini &rarr;</a>
        </div>
    @endif
@endsection